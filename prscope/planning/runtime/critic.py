        ...
        *,
        ...
        try:
            ...
            if mode == "implementability":
                return ImplementabilityResult(
                    implementable=False,
                    missing_details=["LLM unavailable"],
                    implementation_risks=["Reviewer model unavailable during implementability check"],
                    suggested_additions=[],
                    prose="LiteLLM unavailable; implementability fallback used.",
                )
            return ReviewResult(
                strengths=[],
                architectural_concerns=["Reviewer unavailable"],
                risks=[],
                simplification_opportunities=[],
                blocking_issues=["Reviewer model unavailable"],
                reviewer_questions=[],
                recommended_changes=[],
                ...
                primary_issue="Reviewer model unavailable",
                resolved_issues=[],
                constraint_violations=[],
                issue_priority=[],
                prose="LiteLLM unavailable; reviewer fallback used.",
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if strict_mode:
                    raise CriticContractError(f"Reviewer failed in strict mode: {exc}") from exc
                if mode == "implementability":
                    return ImplementabilityResult(
                        implementable=False,
                        missing_details=["Reviewer runtime error"],
                        implementation_risks=[str(exc)],
                        suggested_additions=[],
                        prose=f"Implementability fallback used due to runtime error: {exc}",
                        parse_error=str(exc),
                    )
                return ReviewResult(
                    strengths=[],
                    architectural_concerns=["Reviewer runtime error"],
                    risks=[],
                    simplification_opportunities=[],
                    blocking_issues=["Reviewer runtime error"],
                    reviewer_questions=[],
                    recommended_changes=[],
                    design_quality_score=0.0,
                    confidence="low",
                    review_complete=False,
                    simplest_possible_design=None,
                    primary_issue="Reviewer runtime error",
                    resolved_issues=[],
                    constraint_violations=[],
                    issue_priority=[],
                    prose=f"Reviewer fallback used due to model/runtime error: {exc}",
                    parse_error=str(exc),
                )
            try:
                if mode == "implementability":
                ...
                await self._emit(
                    {
                        "type": "warning",
                        "message": f"Reviewer parse degraded after retries: {exc}",
                    }
                )
                if mode == "implementability":
                    return ImplementabilityResult(
                        implementable=False,
                        missing_details=["Malformed implementability response"],
                ...
                return ReviewResult(
                    strengths=[],
                    architectural_concerns=["Malformed reviewer response"],
                    risks=[],
                    simplification_opportunities=[],
                    blocking_issues=["Malformed reviewer response"],
                    reviewer_questions=[],
                    recommended_changes=[],
                    design_quality_score=0.0,
                    confidence="low",
                    review_complete=False,
                    simplest_possible_design=None,
                    primary_issue="Malformed reviewer response",
                    resolved_issues=[],
                    constraint_violations=[],
                    issue_priority=[],
                    prose=raw,
        ...
        if mode == "implementability":
            return ImplementabilityResult(
                implementable=False,
                missing_details=["Reviewer failed to return implementability contract"],
                implementation_risks=[],
                suggested_additions=[],
                prose="Reviewer failed to return a valid implementability contract.",
                parse_error="unreachable retry guard",
            )
        return ReviewResult(
            strengths=[],
            architectural_concerns=["Reviewer failed to return a valid contract"],
            risks=[],
            simplification_opportunities=[],
            blocking_issues=["Reviewer failed to return a valid contract"],
            reviewer_questions=[],
            recommended_changes=[],
            design_quality_score=0.0,
            confidence="low",
            review_complete=False,
            simplest_possible_design=None,
            primary_issue="Reviewer contract failure",
            resolved_issues=[],
            constraint_violations=[],
            issue_priority=[],
            prose="Reviewer failed to return a valid contract.",
            parse_error="unreachable retry guard",
        )
