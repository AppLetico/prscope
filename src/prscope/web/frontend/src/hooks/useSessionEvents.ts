...
function normalizeEvent(rawType: string, rawPayload: Record<string, unknown>): UIEvent | null {
  ...
  if (rawType === "token_usage") {
    return {
      ...
      llm_call_latency_ms:
        rawPayload.llm_call_latency_ms !== undefined
          ? Number(rawPayload.llm_call_latency_ms)
          : undefined,
      memory_block:
        rawPayload.memory_block !== undefined ? String(rawPayload.memory_block) : undefined,
      session_total_usd:
  ...
  if (rawType === "setup_progress") {
    return {
      type: "setup_progress",
      step: String(rawPayload.step ?? ""),
      draft_stage:
        rawPayload.draft_stage !== undefined ? String(rawPayload.draft_stage) : undefined,
      session_stage:
        rawPayload.session_stage !== undefined ? String(rawPayload.session_stage) : undefined,
      memory_elapsed_s:
        rawPayload.memory_elapsed_s !== undefined ? Number(rawPayload.memory_elapsed_s) : undefined,
      planner_elapsed_s:
        rawPayload.planner_elapsed_s !== undefined
          ? Number(rawPayload.planner_elapsed_s)
          : undefined,
      cache_hit:
        rawPayload.cache_hit !== undefined ? Boolean(rawPayload.cache_hit) : undefined,
      complexity:
